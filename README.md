<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 089 项目横幅" width="1200"></p>
...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 089 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。
...
```text
分析主体、轮廓、姿态与叙事关系 → 确定一个核心主体单元 → 提炼少量动作、局部特征、植物、器物、食物、服饰或材质碎片 → 以漂浮散点布局安排大小、高低、疏密与方向 → 用细线、水彩、彩铅和少量粉蜡笔绘制 → 让手写注释夹在插画之间并保留大量暖白留白
```

## 成品中最容易识别的风格特点

- 主体视觉权重最高，其余元素只是松散呼应；不要平均排布、规则宫格或贴纸商品目录。
- 线条纤细、自然、略带手抖与迟疑；轮廓可以歪斜、断开，颜色允许轻微越线并保留纸张透底。
- 从原图挑选最明亮鲜活的颜色，柔化为清澈蓝、湖水青、嫩绿、奶油黄、杏桃橙、珊瑚粉或柔紫。
- 文字是少量私人手账式 hand-lettered annotation，可夹在插画之间、顺着物件轮廓或斜向散落。
- 拒绝完整场景重绘、复杂背景、粗黑描边、儿童卡通、精确矢量、3D 渲染和商业模板感。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-089-prompt.en.md)
...
| **[xxd-panel-089](https://github.com/nevertoday/xxd-panel-089)** | 私人生活手账 · 漂浮式小品 · 松散手绘线 · 水彩彩铅 · 轻快综合色 · 编辑留白 |
...
| **士兵级** | `xxd-panel-NNN`（当前 001–089） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
