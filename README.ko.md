...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 089이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.

<!-- xxd-human-intro:start -->
## 이 스타일이 해결하는 문제

장면 전체를 다시 그리면 생활 조각의 가벼움이 사라지고, 규칙적인 격자는 스티커 상품 목록처럼 보입니다.

**Panel 089**은 가장 기억할 만한 피사체를 하나의 분명한 주인공 소품 삽화로 두고, 동작·식물·사물·음식·옷·건축·재료·우연한 세부를 몇 개의 보조 조각으로 종이 위에 띄웁니다. 가늘고 느슨한 선, 수채화, 색연필, 소량의 파스텔이 종이와 손의 흔적을 남깁니다.

### 이런 경우에 적합합니다

- 완전한 장면이 아니라 주인공과 기억할 생활 조각을 기록하고 싶을 때
- 크기·높이·밀도·방향이 자연스럽게 달라지는 떠 있는 편집 구성을 원할 때
- 느슨한 선, 수채화, 색연필, 파스텔, 밝고 부드러운 색, 성숙한 여백을 원할 때
- 상하·좌우·디자인 전용·여러 비율·벽지·폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 089로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
**특징:** 개인 생활 다이어리 · 떠 있는 소품 삽화 · 느슨한 손그림 선 · 수채화와 색연필 · 밝고 부드러운 색 · 편집 여백
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 주인공과 생활 조각, 숨 쉬는 여백을 가진 개인 생활 다이어리 한 페이지 |
| **대표 특징** | 떠 있는 소품 삽화, 가는 느슨한 선, 수채화, 색연필, 따뜻한 흰 종이, 밝고 부드러운 색, 손글씨 메모 |
| **입력 존중 방식** | 피사체의 정체성과 서사를 보존하고 장면 전체가 아닌 기억할 세부만 추출함 |
...
```text
피사체·윤곽·자세·서사적 관계 분석 → 핵심 피사체 단위 하나 결정 → 소수의 동작·부분적 특징·식물·기물·음식·의상·건축·재질 조각 추출 → 크기·높낮이·밀도·방향을 달리한 떠 있는 산점형 레이아웃으로 배치 → 가는 선·수채화·색연필·소량의 파스텔로 표현 → 손글씨 메모를 삽화 사이에 엮고 따뜻한 흰 여백을 넉넉히 남김
```

## 완성작의 특징

- 피사체의 시각적 비중이 가장 크고 나머지 조각은 느슨하게 호응합니다. 균등 배치, 규칙적인 격자, 스티커 상품 카탈로그로 만들지 않습니다.
- 가는 선에는 망설임, 흔들림, 끊김을 남기고 종이 바탕, 연필 입자, 얇은 수채화 층, 부분적인 파스텔 마찰감을 드러냅니다.
- 원본에서 가장 밝고 생생한 색을 골라 맑은 파랑, 청록, 연두, 크림 노랑, 살구, 코랄, 부드러운 보라로 유연하게 다듬습니다.
- 소수의 hand-lettered annotation은 동작, 기분, 소리, 냄새, 기억, 우연한 세부에서 생겨나며 삽화 사이 또는 물건의 윤곽을 따라 놓일 수 있습니다.
- 완전한 장면 다시 그리기, 복잡한 배경, 규칙적인 격자, 스티커 UI, 굵은 검은 윤곽선, 어린이 만화, 정밀 벡터, 3D 렌더링, 상업용 템플릿을 피합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-089-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 089의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-089 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-089 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-089 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-089 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-089 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-089 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 089 적용, 공통 설정은 한 번만 확정
/xxd-panel-089 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-089`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-089-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-089-prompt.zh-CN.md)
...
| **[xxd-panel-089](https://github.com/nevertoday/xxd-panel-089)** | 개인 생활 다이어리 · 떠 있는 소품 삽화 · 느슨한 선 · 수채화 · 색연필 · 편집 여백 |
...
| **병사급** | `xxd-panel-NNN` (현재 001–089) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
