<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 089 プロジェクトバナー" width="1200"></p>
...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 089 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。

<!-- xxd-human-intro:start -->
## このスタイルが解決すること

場面全体の描き直しは生活の断片から軽さを奪い、規則的なグリッドはステッカー商品一覧に見えてしまいます。

**Panel 089** は最も記憶に残る主体を一つの明確な主役とし、動作、植物、器物、食べ物、服、建築、素材、偶然の細部を少数の補助断片として紙面に浮かべます。細くゆるやかな線、水彩、色鉛筆、少量のパステルによって、紙の白さ、粒子、わずかな色のはみ出しを残します。

### 向いている用途

- 完全な場面ではなく、主役と記憶に残る生活断片を記録したい場合
- 大小、高低、疎密、方向が自然に変化する浮遊式編集レイアウトが必要な場合
- ゆるい細線、水彩、色鉛筆、パステル、明るく柔らかな色、成熟した余白を求める場合
- 上下・左右・デザインのみ・複数比率・壁紙・フォルダ一括処理

### すぐ試す

> この画像を XXD Panel 089 で処理し、最適な構図とサイズを先に提案してください。
...
**特徴：** 私的な暮らしの手帳 · 浮遊する小品イラスト · ゆるい手描き線 · 水彩と色鉛筆 · 明るく柔らかな色 · 編集的な余白
...
| 確認したいこと | このスタイルの答え |
|---|---|
| **得られるもの** | 主役、生活断片、呼吸する紙面を持つ私的な暮らしの手帳の一頁 |
| **識別しやすい特徴** | 浮遊する小品、細くゆるやかな線、水彩、色鉛筆、暖かな白い紙面、明るく柔らかな色、手書き注釈 |
| **入力の尊重** | 主体の同一性と物語を保ち、場面全体ではなく記憶に残る細部だけを抽出する |
...
```text
主体・輪郭・ポーズ・物語上の関係を分析 → 核となる主体ユニットを一つ決める → 少数の動作・部分的特徴・植物・器物・食べ物・服飾・建築・素材の断片を抽出 → 大小・高低・疎密・方向を変えながら浮遊する散点レイアウトに配置 → 細線・水彩・色鉛筆・少量のパステルで描く → 手書き注釈をイラストの間に織り込み、広いウォームホワイトの余白を残す
```

## 完成作の特徴

- 主体の視覚的な比重を最も高くし、ほかの断片はゆるやかに呼応させる。均等配置、規則的なグリッド、ステッカー商品カタログにはしない。
- 細い線にためらい、揺れ、途切れを残し、紙の地、鉛筆の粒子、水彩の薄い層、部分的なパステルの擦れを見せる。
- 元画像から最も明るく生き生きした色を選び、澄んだ青、青緑、若草色、クリームイエロー、アプリコット、コーラル、柔らかな紫へと柔らかく整える。
- 少数の hand-lettered annotation は、動作、気分、音、匂い、記憶、偶然の細部から生まれ、イラストの間や物の輪郭に沿って配置できる。
- 場面全体の描き直し、複雑な背景、規則的なグリッド、ステッカーUI、太い黒輪郭、子ども向け漫画、精密なベクター、3Dレンダリング、商用テンプレートを避ける。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-089-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 089 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-089 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-089 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-089 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-089 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 089 を適用し、共通設定は一度だけ解決
/xxd-panel-089 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-089`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-089-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-089-prompt.zh-CN.md)
...
| **[xxd-panel-089](https://github.com/nevertoday/xxd-panel-089)** | 私的生活手帳 · 浮遊する小品 · ゆるい線 · 水彩 · 色鉛筆 · 編集的余白 |
...
| **兵士級** | `xxd-panel-NNN`（現在 001–089） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
